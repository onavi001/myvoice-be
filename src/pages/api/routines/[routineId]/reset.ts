  ...
  switch (req.method) {
    case "PUT":
      try {
        ...
        for (const dayData of populatedRoutine.days as IDay[]) {
          const exercises: IExercise[] = Array.isArray(dayData.exercises) && typeof dayData.exercises[0] !== 'string'
            ? (dayData.exercises as IExercise[])
            : [];
